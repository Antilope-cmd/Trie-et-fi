...
def merge_sort(hist_list:list[Histogram]):
    mergesort(hist_list)
    globals.moves_queue.put(("finished",))
    return


def merge(left:list[Histogram], right:list[Histogram], start_of_segment):
    merged = []
    i = 0; j = 0
    ...
    while i < len_left and j < len_right:
        ...
        globals.moves_queue.put(("compare", start_of_segment + i, start_of_segment + len_left + j))

        if left[i].value < right[j].value:
            merged.append(left[i])
            #Because we tracked the start of the segment in the main list with k, we know where the overwrite needs to happen
            globals.moves_queue.put(("set", k, left[i].value))

            i += 1

        else:
            merged.append(right[j])
            globals.moves_queue.put(("set", k, right[j].value))
...
def mergesort(hist_list:list[Histogram], start_of_segment=0):
    ...
    return merge(left, right, start_of_segment)
